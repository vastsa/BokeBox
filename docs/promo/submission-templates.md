...
> BokeBox（播匣）：把看不完 / 听不进的内容，变成真正能听完的私人播客。支持视频、链接、文稿、会议与课程等多种输入；AI 生成口播，人设和音色可自定义；**MCP 让 AI 助手直接创建节目**，**插件式自定义内容源**可扩展抓取，数据只留在你自己的机器上。

**English**

> BokeBox: Turn anything worth finishing — videos, links, articles, meeting notes — into a private podcast. Custom host persona & voice, **MCP tools for AI agents**, and **pluggable source plugins** — fully self-hosted.
...
```markdown
#### vastsa - [Github](https://github.com/vastsa)
* :white_check_mark: [BokeBox 播匣](https://github.com/vastsa/BokeBox)：把看不完/读不完的内容变成通勤可听的私人 AI 播客；支持视频链接文稿等多源输入，人设音色可自定义，MCP + 插件源，本地私有部署 - [更多介绍](https://github.com/vastsa/BokeBox/blob/main/README.zh-CN.md)
```
...
适合通勤、家务、散步时把「看不完 / 读不完 / 整理不完」变成「听得进」。
...
如果你也有一堆永远看不完的视频收藏，又希望用自己的 Agent 工作流接入，这个方向会很共鸣。

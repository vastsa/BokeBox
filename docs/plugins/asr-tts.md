...
宿主 TTS 合成约定：按句号/问号/叹号/换行 **一句一段** 调用 `synthesizeChunk`；句间插入自适应静音（约 0.45–1.2s）；风格/语气标签 **按句规划场景注入**；合成后写时间轴与 `podcast.srt`（`GET /api/jobs/:id/srt`）。
...
完整规范：[TTS 插件开发](../development/tts-plugin.md)
...
详见 [`examples/tts-plugin-fishspeech/README.md`](https://github.com/vastsa/BokeBox/blob/main/examples/tts-plugin-fishspeech/README.md)。

...
- 预置网格：[`examples/tts-plugin-echo`](https://github.com/vastsa/BokeBox/tree/main/examples/tts-plugin-echo)
- 自定义 reference 面板：[`examples/tts-plugin-fishspeech`](https://github.com/vastsa/BokeBox/tree/main/examples/tts-plugin-fishspeech)

总览：[ASR / TTS 插件总览](../plugins/asr-tts.md)
...
3. **语气标签**：按句规划场景标签（用户风格底色 + 句级控制如轻笑/郑重/深呼吸），每句可不同，不限于全文首段
4. **串行合成**后按句长/标点/段落插入自适应句间静音（约 0.45–1.2s），拼接音频，写脚本时间轴，并导出 `podcast.srt`

...
每条订阅 = **订阅插件 + 参数 + 节奏（cron）**，可选时区；内容采集默认 **自动匹配 Source 插件**，也可在设置里固定。
...
- **立即执行 / 强制执行**：调试或补跑  
- **运行记录**：每轮写入 `schedule_runs`（状态 / 抓取数 / 新建任务 / 跳过 / 错误 / jobIds）；设置页可展开详情并跳转任务  
- **失败可重试**：仅成功建 Job 的条目记入去重；失败条目下轮可再试，也可用强制执行忽略去重  

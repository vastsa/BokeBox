...
默认直接在**当前工作区与当前分支**完成开发、验证和提交，**不再要求为每个任务创建独立 git worktree**。

### 必须遵守

- 开始修改前先检查 `git status`，识别并保留用户已有改动
- 只修改和提交当前任务相关文件，不覆盖、不清理、不混入无关改动
- 功能开发、修复和重构仍需完成自检与验证后再提交
- 多项工作确实需要并行且会产生文件冲突时，才考虑拆分分支或 worktree
- 仅在用户明确要求时创建、合并或清理 worktree

### 禁止

- 为普通单任务开发默认创建 worktree
- 擅自处理、暂存或提交当前工作区中的无关改动
- 未验证就直接提交大范围修改

...
const MIMO_TAG_RULES_EN = [
  ...
  '   - Keep tag tokens as Chinese control words; parentheses wrap control words only, never full sentences.',
  '   - Global base: optional 1 style tag at the start, e.g. (沉稳) / (磁性) / (欢快).',
  '     Style words: 磁性/沉稳/温柔/慵懒/怅然/深情/欢快/激昂/清亮/甜美.',
  '   - Sentence-level tone: tag key sentences by scene so delivery matches context, not one tone for the whole script.',
  '     Mapping examples: opening→（深呼吸）; key conclusion→（郑重）/（提高音量）; surprise→（震惊）;',
  '     light joke→（轻笑）; regret→（叹气）; quick list→（语速加快）; closing→（轻笑）.',
  '   - Fine-tag examples: （深呼吸）（轻笑）（沉默片刻）（语速加快）（小声）（提高音量）（叹气）（哽咽）（郑重）（激动）.',
  '   - Prioritize emotional turns and structure nodes (open/turn/point/close); plain transitions may skip tags.',
  '   - About 10-20 fine tags total; consecutive sentences may use different tags, but do not spam the same tag.',
  '   - Good example: (沉稳)Hello and welcome.（深呼吸）One key takeaway today.（郑重）Execution decides the outcome.（轻笑）See you next time.',
];
